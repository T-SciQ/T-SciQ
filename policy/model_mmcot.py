...
# from moe_models import T5Stack, __HEAD_MASK_WARNING_MSG
import copy
import math
import os
import warnings
...
class T5ForMultimodalGeneration(T5ForConditionalGeneration):
    ...
    def forward(
        ...
        # encoder_counts = encoder_outputs.counts
        # encoder_route_probs = encoder_outputs.route_probs
        ...
        # decoder_counts = decoder_outputs.counts     # (12, 3)
        # decoder_route_probs = decoder_outputs.route_probs   # (12, 3)
        ...
        if labels is not None:
            ...
            # # moe balance loss
            # counts = torch.concat([encoder_counts, decoder_counts], 0).sum(0)
            # route_probs = torch.concat([encoder_route_probs, decoder_route_probs], 0).sum(0)
            # total = counts.sum(-1, keepdim=True)
            # f = counts / total
            # p = route_probs / total
            # balance_loss = self.n_experts * (f * p).sum()
            # loss = loss + self.alpha * balance_loss
